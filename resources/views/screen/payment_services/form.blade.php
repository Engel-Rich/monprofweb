            ...
            <form method="POST" enctype="multipart/form-data" action="{{ $isEdit ? route('admin.payment-services.update', $service) : route('admin.payment-services.store') }}">
                ...
                <article class="admin-panel form-panel payment-routing-panel">
                    ...
                    <div class="entity-field-grid two">
                        ...
                        <div class="entity-field"><label for="provider_fee_percentage">Frais du fournisseur (%) <em>Obligatoire</em></label><input id="provider_fee_percentage" name="provider_fee_percentage" type="number" min="0" max="100" step="0.0001" value="{{ old('provider_fee_percentage', $service->provider_fee_percentage ?? 2.5) }}" required><small>Ex. 2,5 pour des frais fournisseur de 2,5 %.</small>@error('provider_fee_percentage')<span class="field-error">{{ $message }}</span>@enderror</div>
                        <div class="entity-field"><label for="user_fee_percentage">Part payée par l’utilisateur (%) <em>Obligatoire</em></label><input id="user_fee_percentage" name="user_fee_percentage" type="number" min="0" max="100" step="0.01" value="{{ old('user_fee_percentage', $service->user_fee_percentage ?? 100) }}" required><small>100 % facture tous les frais au client ; 0 % les prend entièrement en charge.</small>@error('user_fee_percentage')<span class="field-error">{{ $message }}</span>@enderror</div>
