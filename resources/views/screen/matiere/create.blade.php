...
<div class="container py-5 px-5">
    <div class="row">
        <div class="col-lg col-md">
            <form method="POST"
                action="{{ $matiere == null ? route('matiere.store') : route('matiere.update', $matiere->id) }}">
                @csrf
                @if ($matiere != null)
                @method('PUT')
                @endif
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="libelle" aria-describedby="classe_name"
                        name="libelle" value="{{ old('libelle', $matiere?->libelle) }}" required>

                </div>
                <div class="mb-3">
                    <label class="form-label fw-bold" for="app_name">Nom reservé</label>
                    <input type="text" class="form-control" id="app_name" name="app_name"
                        value="{{ old('app_name', $matiere?->app_name) }}" required>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="description" name="description"
                        value="{{ old('description', $matiere?->description) }}">
                </div>
            ...
            @if(isset($matiere))
            <div class="py-2">
                <form action="{{ route('matiere.destroy', $matiere) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn btn-outline-danger px-5"
                        onclick="return confirm('Voulez-vous vraiment supprimer cette matiere ?')">
                        Supprimer
                    </button>
                </form>
            </div>
