...
@section('content')
    <h1 class="display-5">
        {{ $classe->libelle == null ? 'Ajouter une nouvelle classe' : 'Modifier la classe ' . $classe->libelle }}</h1>

    <div class="container py-5 px-5">
        <div class="row">
            <div class="col-lg col-md">
                <form method="POST"
                    action="{{ $classe->id == null ? route('classe.store') : route('classe.update', $classe->id) }}">
                    @csrf
                    @if ($classe->id != null)
                        @method('PUT')
                    @else
